function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;} // Using the Random User API: https://randomuser.me/api/


class Search extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      searchText: '' };

    this.handleChange = this.handleChange.bind(this);
  }

  handleChange(e) {
    this.setState({ searchText: e.target.value }, () => {
      this.props.searchText(this.state.searchText);
    });
  }

  render() {
    return /*#__PURE__*/(
      React.createElement("div", null, /*#__PURE__*/
      React.createElement("form", { className: "Search", onSubmit: e => e.preventDefault() }, /*#__PURE__*/
      React.createElement("input", { type: "text", className: "Search-box", placeholder: "Filter names", onChange: this.handleChange }))));



  }}


class User extends React.Component {












  render() {
    const { name, email, pic, username } = this.props;
    return /*#__PURE__*/(
      React.createElement("div", { className: "UserCard" }, /*#__PURE__*/
      React.createElement("div", { className: "UserCardTop" }, /*#__PURE__*/
      React.createElement("img", { src: pic })), /*#__PURE__*/

      React.createElement("div", { className: "UserCardBottom" }, /*#__PURE__*/
      React.createElement("h3", null, name), /*#__PURE__*/
      React.createElement("p", null, "email"), /*#__PURE__*/
      React.createElement("h5", null, email), /*#__PURE__*/
      React.createElement("p", null, "username"), /*#__PURE__*/
      React.createElement("h5", null, username))));



  }}_defineProperty(User, "defaultProps", { name: 'John Doe', email: 'JohnDoe@example.com', pic: 'picture.jpg' });_defineProperty(User, "propTypes", { name: PropTypes.string, email: PropTypes.string, pic: PropTypes.string });


class NewUser extends React.Component {constructor(...args) {super(...args);_defineProperty(this, "getNewUser",







    e => {
      fetch('https://randomuser.me/api/').
      then(response => response.json()).
      then(data => {
        let firstName = data.results[0].name.first[0].toUpperCase() + data.results[0].name.first.slice(1);
        let lastName = data.results[0].name.last[0].toUpperCase() + data.results[0].name.last.slice(1);
        let name = firstName + ' ' + lastName;
        let email = data.results[0].email;
        let pic = data.results[0].picture.large;
        let username = data.results[0].login.username;
        this.props.gettingUser({ name, email, pic, username });
      });
    });}

  render() {
    return /*#__PURE__*/(
      React.createElement("button", {
        className: "NewUserButton",
        onClick: this.getNewUser }, /*#__PURE__*/

      React.createElement("h1", { style: { fontSize: '5rem' } }, "?"), /*#__PURE__*/
      React.createElement("h2", null, "Get random new user"), /*#__PURE__*/
      React.createElement("p", {
        style: {
          color: '#BDBDBD',
          padding: '10px 0 20px',
          margin: '10px' } }, "https://randomuser.me/api/")));






  }}_defineProperty(NewUser, "defaultProps", { name: 'Bob', email: 'default@email.com', pic: 'default.jpg' });


class App extends React.Component {
  constructor(props) {
    super(props);_defineProperty(this, "gettingUser",

























    res => {
      const users = [...this.state.users, res];
      this.setState({ users });
    });_defineProperty(this, "searchText",

    res => {
      this.setState({ searchText: res });
    });this.state = { users: [{ name: 'Naruto Uzumaki', email: 'hokage@konoha.gov', pic: 'https://vignette.wikia.nocookie.net/naruto/images/4/42/Naruto_Part_III.png/revision/latest/scale-to-width-down/300?cb=20180117103539', username: 'foxboii' }, { name: 'Sasuke Uchiha', email: 'unknown', pic: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTMrmfXsmvbR1rYFRFwX_2AEc7127qgeB86_eoLd7b9Mn0qfaUx', username: 'ninja' }, { name: 'Himawari Uzumaki', email: 'kawaii@naruto.net', pic: 'data:image/jpeg;base64,', username: 'foxchan' }], searchText: '' };}

  render() {
    return /*#__PURE__*/(
      React.createElement("div", { className: "App" }, /*#__PURE__*/
      React.createElement(Search, { searchText: this.searchText }), /*#__PURE__*/
      React.createElement("main", { className: "listOfCards" },

      this.state.users.
      filter((user) =>
      user.name.toLowerCase().includes(this.state.searchText.toLowerCase())).

      map((user, index) => /*#__PURE__*/
      React.createElement(User, { key: index, name: user.name, email: user.email, pic: user.pic, username: user.username })), /*#__PURE__*/


      React.createElement(NewUser, { gettingUser: this.gettingUser }))));



  }}



ReactDOM.render( /*#__PURE__*/React.createElement(App, null), document.querySelector('#root'));